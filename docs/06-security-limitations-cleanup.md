...
- least-privilege PAT, 30일 만료, token 비노출 원칙을 다시 확인할 수 있다.
...
- PAT는 반드시 **least-privilege**로 유지하고, 실습 기준처럼 필요한 repository만 선택하세요.
- Fine-grained PAT는 **30일 만료**를 기본선으로 두고 장기 토큰을 피하세요.
- PAT, registration token, remove token은 `echo`, 로그, 스크린샷, Git 기록에 출력하지 마세요.
...
- 실습에서는 ACA secret을 사용했지만 production에서는 Azure Key Vault secret reference와 audit trail을 우선 고려하세요.
...
| 항목 | 확인 내용 |
|------|-----------|
| workflow 정리 | 더 이상 필요 없다면 `.github/workflows/aca-runner-scale-test.yml` 또는 lab workflow를 삭제합니다. |
| stale runner 정리 | **Settings → Actions → Runners**에서 오래 남은 offline runner record가 있는지 확인합니다. |
| PAT 폐기 | 실습용 Fine-grained PAT를 revoke 또는 delete 합니다. |
| 저장소 보존 여부 | lab repository 자체가 불필요하면 archive 또는 삭제를 검토합니다. |
...
| 새 workflow가 갑자기 401/403을 반환 | PAT 만료 또는 revoke | 실습이 아직 끝나지 않았다면 새 Fine-grained PAT를 다시 만들고 Job secret을 갱신합니다. 이미 종료 단계라면 revoke/delete 상태를 정상으로 간주합니다. |
