...
grep -Fx 'Module 06은 선택이며, 90분 핵심 경로가 필요하면 [현재 정리 문서](06-security-limitations-cleanup.md)로 바로 이동해도 됩니다.' "$DOC" >/dev/null ||
  fail "module 05 missing explicit cleanup shortcut link"
if grep -F 'cleanup용 Module 07' "$DOC" >/dev/null; then
  fail "module 05 still points cleanup to Module 07 before the rename lands"
fi
